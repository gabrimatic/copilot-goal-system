...
export function buildDriftEnforcement(driftCount, threshold = 5) {
  return [
    ...
    "The current tool call is allowed so the session can recover. At the next useful checkpoint, call goal_system_update with verified doneSoFar, inspectionEvidence or verificationResults, and remaining/blockers. If goal_system_update is unavailable, keep a concise manual checkpoint instead of blocking tool use.",
  ].join(" ");
}
