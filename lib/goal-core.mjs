...
export const ACTIVATION_REGEX =
  /\b(?:\/goal|new goal|goal mode|turn this into a goal|keep working until this is done|continue the active goal|make sure everything is fixed|no escape|do it fully|polish everything|deeply inspect and fix|verify and prove it|reach perfection|nothing left behind)\b/i;
...
export function normalizeCwd(cwd) {
  return path.resolve(normalizeText(cwd, 2000) || process.cwd());
}
...
export function isGoalSystemToolName(toolName) {
  return GOAL_SYSTEM_TOOL_NAMES.has(normalizeText(toolName, 120));
}
...
export function buildDriftEnforcement(driftCount, threshold = 5) {
  return [
    `Goal-state drift guard: ${driftCount} tool calls have run since the last goal_system_update.`,
    `The allowed threshold is ${threshold}.`,
    "Before any other non-goal tool call, call goal_system_update with verified doneSoFar, inspectionEvidence or verificationResults, and remaining/blockers.",
  ].join(" ");
}
...
export function createGoalRecord(args = {}, sessionId, cwd, extra = {}) {
  ...
  return {
    ...
    objective: normalizeText(args.objective) || "unknown until inspected",
    ...
    completionStatus: GOAL_STATUSES.includes(args.completionStatus) ? args.completionStatus : "active",
    history: [
      {
        at: timestamp,
        type: "open",
...
export function mergeGoal(existingGoal, patch = {}, historyType = "update", historyNote = "Goal state updated") {
  ...
  for (const field of ["objective"]) {
    if (typeof patch[field] === "string" && normalizeText(patch[field])) {
      nextGoal[field] = normalizeText(patch[field]);
    }
  }
...
export function getOutstandingIssues(goal) {
  const discovered = new Set(normalizeUniqueList(goal?.discoveredIssues));
  for (const resolved of normalizeUniqueList(goal?.resolvedIssues)) {
    discovered.delete(resolved);
  }
  return [...discovered];
}
...
export function validateGoalCompletion(goal) {
  ...
  if (!inspectionEvidence.length && !toolHistory.some((entry) => isInspectionToolNote(entry.note))) {
  ...
  const hasExplicitWorkEvidence =
    ...
    doneSoFar.length >= 1 &&
    (inspectionEvidence.length >= 1 || toolHistory.some((entry) => isInspectionToolNote(entry.note)));
...
export function buildGoalPromptNote(goal) {
  ...
  return [
    ...
    "Before claiming completion, reload goal_system_status and close only with real inspection evidence, resolved issues, validation proof, verification results, and a completion audit.",
  ].join("\n");
}
...
export function summarizeToolUse(input = {}) {
  const toolName = normalizeText(input.toolName || input.name, 120);
  const args = input.toolArgs || input.arguments || {};
  if (!toolName) return "";
...
export class GoalStore {
  ...
  auditLog(event, data = {}) {
    ...
    appendFile(this.logPath, line, "utf8").catch(() => {});
  }
  ...
  pickSingleOpenWorkspaceGoal(records) {
    const openRecords = records.filter((record) => isOpenGoal(record.goal));
  ...
  async writeCompactSnapshot(sessionId, cwd, goal) {
    ...
    await this.writeJsonAtomic(`${snapshotPath}.json`, { snapshot, goalId: goal.id, sessionId: safeSessionId(sessionId), cwd: normalizeCwd(cwd), at: nowIso() });
    return snapshot;
  }
}
