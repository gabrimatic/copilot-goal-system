...
export function buildDriftEnforcement(driftCount, threshold = 5) {
  return [
    ...
    "The current tool call is allowed so the session can recover. At the next useful checkpoint, call goal_system_update with verified doneSoFar, inspectionEvidence or verificationResults, and remaining/blockers. If direct goal tools are unavailable, use local goalctl with the current sessionId and cwd instead of blocking tool use.",
  ].join(" ");
}
...
export function buildGoalPromptNote(goal) {
  ...
  return [
    ...
    "Before claiming completion, reload goal_system_status, or local goalctl status if direct tools are unavailable, and close only with real inspection evidence, resolved issues, validation proof, verification results, and a completion audit.",
  ].join("\n");
}
...
export function buildStopContinuationDirective(goal) {
  ...
  return [
    ...
    "1. Call goal_system_status to reload authoritative state, or run local goalctl status with the current sessionId and cwd if direct tools are unavailable.",
    ...
    "3. Call goal_system_update, or local goalctl update, after meaningful inspection, fixes, blockers, verification, or remaining-work changes.",
    "4. Call goal_system_close, or local goalctl close, only when completion, blockage, or cancellation is supported by exact evidence and the completion audit passes.",
  ].join("\n");
}
