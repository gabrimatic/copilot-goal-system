...
export function buildDriftEnforcement(driftCount, threshold = 5) {
  return [
    ...
    "The current tool call is allowed so the session can recover. At the next useful checkpoint, call goal_system_update with verified doneSoFar, inspectionEvidence or verificationResults, and remaining/blockers. If direct goal tools are unavailable, run local goalctl as a command with the current sessionId and cwd; do not read or inspect the goalctl implementation just to use goal state.",
  ].join(" ");
}
...
export function buildGoalPromptNote(goal) {
  ...
  return [
    ...
    "Before claiming completion, reload goal_system_status, or run local goalctl status if direct tools are unavailable. Treat goalctl as a command API, not source to inspect. Close only with real inspection evidence from the user-requested target, resolved issues, validation proof, verification results, and a completion audit.",
  ].join("\n");
}
