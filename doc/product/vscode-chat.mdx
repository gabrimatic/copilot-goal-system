...
| `~/.copilot/extensions/goal-system/bin/goalctl.mjs` | Local no-MCP command fallback for goal operations. |
...
If direct tools are unavailable, run the same operation through `goalctl` with those values.
