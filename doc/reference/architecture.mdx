...
Prompt starts /goal
  ...
  -> SDK tool, MCP tool, VS Code tool, or goalctl opens or checkpoints persisted goal state
...
CLI and VS Code hooks inject `sessionId` and `cwd` into the session context. Direct tools, MCP tools, and `goalctl` require those values as a cooperative isolation convention so multiple sessions in one workspace stay out of each other's way during normal use; see [Isolation model](#isolation-model) for why this is not a security boundary. Persisted tool history drives drift enforcement across hook invocations.
