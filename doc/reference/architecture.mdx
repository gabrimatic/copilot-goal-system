...
CLI and VS Code hooks inject `sessionId` and `cwd` into the session context. Direct tools, MCP tools, and `goalctl` require those values so multiple sessions in one workspace stay isolated. Persisted tool history drives drift enforcement across hook invocations.
