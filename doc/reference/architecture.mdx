...
CLI and VS Code hooks inject `sessionId` and `cwd` into the session context. MCP tools require those values so multiple sessions in one workspace stay isolated. Persisted tool history drives drift enforcement across hook invocations.
...
`goal_system_status`, `goal_system_open`, `goal_system_update`, and `goal_system_close` do not count toward drift, including Copilot CLI MCP-prefixed tool names such as `goalSystem-goal_system_update`.
...
- MCP goal tools require the main session `sessionId` and `cwd` from hook context.
