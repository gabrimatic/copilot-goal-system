...
Current installs use local hooks, direct tools, and `goalctl`; they do not create server config.
...
Current installers do not write any server config. Uninstalling only needs to remove the installed goal files, hook entries, and instruction snippet above.
