---
title: "Requirements"
description: "The goal-system contract and the evidence required before completion."
---
...
- Drift control: the main session must update state after meaningful work, and stale progress must produce escalating recovery reminders without trapping the session when update tools are unavailable.
...
`goal_system_finish` may mark a goal complete only when all of these are true:
