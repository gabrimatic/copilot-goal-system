...
Copilot Goal System stores local goal state and injects compact context into GitHub Copilot CLI sessions. It does not send telemetry, analytics, prompts, or goal state to a service owned by this project.

Network behavior is limited to install-time dependency retrieval through `npm`, and whatever GitHub Copilot CLI itself does in your environment.
...
|---------|------------|-------|
| Installer | Filesystem writes | `~/.copilot/extensions/goal-system`, `~/.copilot/skills/goal`, `~/.copilot/hooks`, `~/.copilot/settings.json`, `~/.copilot/copilot-instructions.md` |
| Extension | Filesystem writes | Goal state and audit log under `~/.copilot/session-state/goal-system` |
| Hook | Filesystem reads/writes | Reads goal state, writes compact snapshots |
...
- Issues in GitHub Copilot CLI itself.
